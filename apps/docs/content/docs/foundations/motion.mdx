...
PDS의 모션은 **pluto-note chat-animations를 승계**했다. `--pds-ease-standard`가 모든 step-in·fade·collapse의 기본 이징이며, `--pds-chat-easing`은 같은 값의 alias다.
