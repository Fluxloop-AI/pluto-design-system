...
Montage 에는 독립 Icon 컴포넌트가 없어 **신규 설계**. @phosphor-icons/react 의 아이콘 컴포넌트를 `icon` prop 으로 주입하고 PDS 기본 size / color 를 적용한다. 공급자는 `@fluxloop-ai/pds-icons/icons` 중립 서브패스로만 소비한다.
