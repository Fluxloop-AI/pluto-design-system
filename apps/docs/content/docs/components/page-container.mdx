...
기본 variant 는 `default` (720px). 본문이 가운데 정렬되고 좌우 32px gutter, 상단 32px / 하단 64px padding 이 적용된다.

<PageContainerBasicDemo />

## Variants

페이지 콘텐츠 성격에 따라 4가지 max-width 변종을 고른다. gutter 32 와 상하 padding 32/64 는 모든 variant 에서 동일.

<PageContainerVariantsDemo />

| variant | max-width | 용도 |
|---|---|---|
| `narrow` | `640px` | 단일 컬럼 텍스트·폼·문서. 한 줄 가독성 (대략 65~75자) |
...
| Prop | 타입 | 기본 | 설명 |
|---|---|---|---|
| `variant` | `"narrow" \| "default" \| "wide" \| "full"` | `"default"` | 본문 max-width 변종 |
