...
>
> 다음 11개 컴포넌트는 **실험 단계**입니다. 1.0 이후 재설계가 예정되어 있어 API 가 자주 깨질 수 있습니다.
> 프로덕션 채택 전에 변경 이력을 반드시 확인하세요.
>
> `ChatBlock` · `ChatBubble` · `ChatComposer` · `ChatLoadingDots` · `ChatStepDot` ·
> `ChatTabBar` · `ChatThread` · `AgentStatusIndicator` · `ThinkingBlock` ·
> `ToolCallCard` · `ToolCodeBlock`
>
